...
;; thread. return-key-list is an optional argument where the user can 
...
;; (go env example-coroutine (list (list '#:data key1)
;;                                 (list '#:data key2)))
;;
;;
...
;; '#:message type source
;; '#:channel channel
;; '#:data key datapool ;if datapool is unspecified use container datapool
;;
;; The actual values provided for each message/data/channel handler 
...
;; (list (list '#:data key) (list '#:message type source))
;; (list (list '#:channel ch1) (list '#:channel ch2))
;; (list (list '#:data key1 dp2) 
;;       (list '#:data key2 dp2) 
...
;;   example-env 
;;   example-handler 
;;   (data-changed-type example-key)                  ;<== when we get this message 
;;   #f                                                              ;<== from this source
;;   (list (list example-input-key))            ;<== give these data values (and the message) as input into example-handler
;;   (list (list '#:data ex-destination-key))) ;<== use these to direct where the output from example-handler is stored
(define (data-changed-type key)
  (let ([o (open-output-string)])
...
;;;---------------------------------------------------------------------------- 
;; message record 
(define-record message (source type content))
...
;;
;;
;; When the callback activates the data stored at each key specified 
...
;;
;;
;; input-data should be a list of key like the following:
;; (list key1 key2 ... keyn)
;;
;;
...
;;   (data-changed-type example-key)           ;<== when we get this message 
;;   #f                                        ;<== from this source
;;   (list example-input-key)                  ;<== give these data values (and the message) as input into example-handler
;;   (list (list '#:data ex-destination-key))) ;<== use these to direct where the output from example-handler is stored
...
;; return list of data obtained from datapool based on given key
(define (get-input-data env input-key-list)
  (map 
    (lambda (data-key) 
      (get-data env data-key))
    input-key-list))
    ...
    (if handlers
        (let ()
          (map
            (lambda (i)
              (let ([handler (car i)]
                    [input-key-list (cadr i)]
                    [return-key-list (caddr i)])
                (if input-key-list 
                    ;if input-key-list exists, grab the data and pass to 
                    ;handler coroutine alongside the message
                    (go 
                      env 
                      (handler msg (get-input-data env input-key-list)) 
                      return-key-list)
                    ;otherwise just pass the message to the coroutine
                    (go env (handler msg) return-key-list))))
            handlers)
          #t)
        #f)))
...
(define (handle-go-returns env return-dests return-vals)
  ...
  ;Business logic for message return
  (define (handle-message dest val)
  ...
  (define (handle-return dest val)
    (let ([return-type (car dest)])
      (if (equal? val 'nil)
          (values) ;do nothing
          (cond [(or (eqv? 'message return-type) (eqv? 'm return-type))
                 (handle-message dest val)]
                [(or (eqv? 'data return-type) (eqv? 'd return-type))
                 (handle-data dest val)]
                [(or (eqv? 'channel return-type) (eqv? 'c return-type))
                 (handle-channel dest val)]
